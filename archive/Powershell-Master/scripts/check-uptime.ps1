...
.EXAMPLE
	PS> ./check-uptime.ps1
	✅ COMPUTER01 is up for 13 days since 1/25/2024
.LINK
...
try {
	...
	if ($IsLinux) {
		...
		if (Test-RegistryValue -Path $RegistryPaths["ComponentRebootProgress"] -Value "RebootInProgress") {
			$reason += ", Component Based Servicing with RebootInProgress"
		}
		if (Test-RegistryValue -Path $RegistryPaths["ComponentRebootProgress"] -Value "PackagesPending") {
			$reason += ", Component Based Servicing with PackagesPending"
		}
		if (Test-RegistryValue -Path $RegistryPaths["SessionManager"] -Value "PendingFileRenameOperations2") {
			$reason += ", Session Manager with PendingFileRenameOperations2"
		}
		if (Test-RegistryValue -Path $RegistryPaths["RunOnce"] -Value "DVDRebootSignal") {
			$reason += ", RunOnce with DVDRebootSignal"
		}
		if (Test-RegistryValue -Path $RegistryPaths["NetlogonServices"] -Value "JoinDomain") {
			$reason += ", Netlogon Services with JoinDomain"
		}
		if (Test-RegistryValue -Path $RegistryPaths["NetlogonServices"] -Value "AvoidSpnSet") {
