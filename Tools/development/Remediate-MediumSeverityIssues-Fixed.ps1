...
# Case-insensitive patterns for upper case variations
$uppercasePatterns = @{
    'C:\\TEMP\\'          = '(Join-Path $env:TEMP "")'
...
function Repair-HardcodedPaths {
    ...
    # Process uppercase patterns
    foreach ($pattern in $uppercasePatterns.GetEnumerator()) {
        if ($content -match [regex]::Escape($pattern.Key)) {
            $content = $content -replace [regex]::Escape($pattern.Key), $pattern.Value
            $modified = $true
            Write-Host "  📁 Fixed hardcoded path: $($pattern.Key) → $($pattern.Value)" -ForegroundColor Green
...
# Function to repair insecure protocols
function Repair-InsecureProtocols {
    param([string]$FilePath)
...
if (Test-Path $reportPath) {
    ...
    $fileMatches = [regex]::Matches($htmlContent, '<h3>📄 ([^<]+)</h3>.*?MEDIUM', [System.Text.RegularExpressions.RegexOptions]::Singleline)
    $mediumSeverityFiles = $fileMatches | ForEach-Object { $_.Groups[1].Value.Trim() } | Sort-Object -Unique
}
...
foreach ($filePath in $mediumSeverityFiles) {
    ...
    try {
        switch ($IssueType) {
            "HardcodedPaths" {
                if (Repair-HardcodedPaths -FilePath $filePath) {
                    $fileModified = $true
                    $totalIssuesFixed++
                }
            }
            "InsecureProtocols" {
                if (Repair-InsecureProtocols -FilePath $filePath) {
                    $fileModified = $true
                    $totalIssuesFixed++
                }
            }
            "All" {
                if (Repair-HardcodedPaths -FilePath $filePath) { $fileModified = $true; $totalIssuesFixed++ }
                if (Repair-InsecureProtocols -FilePath $filePath) { $fileModified = $true; $totalIssuesFixed++ }
            }
        }
        ...
        if (-not $auditEntry.ContainsKey('Errors')) { $auditEntry.Errors = @() }
