...
#   gc $f | ? {$_ -notmatch } | % {$_ -replace '(^.*?)\s*?[^``]#.*','$1'} | Out-File $f+ -en utf8; mv -fo $f+ $f
...
$packageArgs = @{
  ...
  #silentArgs   = '/s /v'   # InstallShield with MSI
...
# take a dependency on  in your nuspec file.
...
if ($key.Count -eq 1) {
  $key | % {
    $packageArgs['file'] =  #NOTE: You may need to split this if it contains spaces, see below

    if ($packageArgs['fileType'] -eq 'MSI') {
      # The Product Code GUID is all that should be passed for MSI, and very
      # FIRST, because it comes directly after /x, which is already set in the
      # Uninstall-ChocolateyPackage msiargs (facepalm).
      $packageArgs['silentArgs'] = 
      ...
      # - Ensure you don't pass double quotes in $file (aka $packageArgs['file']) - otherwise you will get 
