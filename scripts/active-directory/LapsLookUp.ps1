...
# NOTE: These are Microsoft-defined Active Directory LAPS attribute names, not actual passwords
$LAPSAttributes = @{
    Password        = 'ms-Mcs-Admpwd'                      # AD attribute for LAPS password (not a literal password)
    PasswordExpiry  = 'ms-Mcs-AdmPwdExpirationTime'   # AD attribute for password expiration
    PasswordHistory = 'ms-Mcs-AdmPwdHistory'         # AD attribute for password history
    PasswordLength  = 'ms-Mcs-AdmPwdLength'           # AD attribute for password length
}

# 🔐 SECURITY VALIDATION: Verify LAPS attributes are not exposed
Write-AuditLog -Message "LAPS attribute lookup initialized - accessing AD schema definitions only" -Severity "Info"
